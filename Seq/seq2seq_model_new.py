...
class Seq2SeqModel(object):
    ...
    def init_placeholders(self):       
        # encoder_inputs: [batch_size, max_time_steps, input_size]
        self.encoder_inputs = tf.placeholder(dtype=tf.float32,
            shape=(None, None, self.input_size), name='encoder_inputs')
        ...
        if self.mode == 'train':
            ...
            decoder_start_token = tf.ones(
                shape=[self.batch_size, 1], dtype=tf.int32) * 0 # start_token_p
            decoder_end_token = tf.ones(
                shape=[self.batch_size, 1], dtype=tf.int32) * 1 # end_token_p          
    ...
    def build_decoder(self):
        ...
        with tf.variable_scope('decoder'):
            ...
            if self.mode == 'train':
                ...
                start_tokens = tf.ones([self.batch_size,], tf.int32) * 0 # start_token_p
                decode_end_token = 1 # end_token_p
